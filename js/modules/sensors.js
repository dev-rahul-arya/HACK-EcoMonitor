...
class SensorSimulator {
    ...
    generateSensorData() {
        ...
        // Simulate time-based variations (pollution higher during rush hours, etc.)
    ...
    getWeatherCondition() {
        const conditions = [
            { name: 'Clear', weight: 30 },
            { name: 'Partly Cloudy', weight: 25 },
            { name: 'Cloudy', weight: 15 },
            { name: 'Overcast', weight: 10 },
            { name: 'Light Rain', weight: 8 },
            { name: 'Rain', weight: 5 },
            { name: 'Thunderstorm', weight: 3 },
            { name: 'Fog', weight: 2 },
            { name: 'Haze', weight: 2 }
        ];
    ...
    updateSensorReadings(data) {
        this.sensors.forEach(sensor => {
            // Randomly toggle some sensors offline occasionally
            if (Math.random() < 0.02) {
            ...
            if (sensor.status === 'offline' && Math.random() < 0.1) {
                sensor.status = 'online';
            }

            // Set last reading based on sensor type
            switch (sensor.type) {
                case 'air':
                    sensor.lastReading = `AQI: ${data.air.aqi}`;
                    break;
                case 'water':
                    sensor.lastReading = `pH: ${data.water.ph}`;
                    break;
                case 'weather':
                    sensor.lastReading = `${data.weather.temperature}°C`;
                    break;
                case 'environmental':
                    sensor.lastReading = `${data.weather.humidity}% RH`;
                    break;
            }
        });
    }
    ...
    generateForecast() {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (hourOfDay >= 6 && hourOfDay <= 14) {
                tempVariation = (hourOfDay - 6) * 0.5; // Rising
            } else if (hourOfDay > 14 && hourOfDay <= 20) {
                tempVariation = 4 - (hourOfDay - 14) * 0.6; // Falling
            } else {
                tempVariation = -2; // Night time cooler
            }
    ...
    detectAnomalies(data) {
        ...
        if (data.air.aqi > 150) {
            anomalies.push({
                type: 'air',
                severity: data.air.aqi > 200 ? 'critical' : 'warning',
                message: `High AQI detected: ${data.air.aqi}`,
                value: data.air.aqi,
                threshold: 150,
        ...
        if (parseFloat(data.air.pm25) > 35) {
            anomalies.push({
                type: 'air',
                severity: parseFloat(data.air.pm25) > 55 ? 'critical' : 'warning',
                message: `Elevated PM2.5 levels: ${data.air.pm25} μg/m³`,
                value: parseFloat(data.air.pm25),
                threshold: 35,
                timestamp: data.timestamp
            });
        }
        ...
        if (parseFloat(data.weather.temperature) > 38) {
            anomalies.push({
                type: 'weather',
                severity: 'warning',
                message: `Extreme heat warning: ${data.weather.temperature}°C`,
                value: parseFloat(data.weather.temperature),
                threshold: 38,
                timestamp: data.timestamp
            });
        }
        
        // Check water pH anomaly
        const ph = parseFloat(data.water.ph);
        if (ph < 6.5 || ph > 8.5) {
            anomalies.push({
                type: 'water',
                severity: (ph < 6.0 || ph > 9.0) ? 'critical' : 'warning',
                message: `Abnormal water pH: ${data.water.ph}`,
                value: ph,
                threshold: '6.5-8.5',
                timestamp: data.timestamp
            });
        ...
        if (parseFloat(data.water.dissolvedOxygen) < 6) {
            anomalies.push({
                type: 'water',
                severity: parseFloat(data.water.dissolvedOxygen) < 4 ? 'critical' : 'warning',
                message: `Low dissolved oxygen: ${data.water.dissolvedOxygen} mg/L`,
                value: parseFloat(data.water.dissolvedOxygen),
                threshold: 6,
                timestamp: data.timestamp
            });
        }
